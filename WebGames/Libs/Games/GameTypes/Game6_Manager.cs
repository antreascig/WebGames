    ...
    public class Game6_Manager
    {
        ...
        public static List<Game6_UserScore_Dto> GetUsersScore()
        {
            ...
            using (var db = ApplicationDbContext.Create())
            {
                ...
                if (Game != null)
                {
                    res = (from gs in db.Game6_Scores select gs).Select(gs => GenerateUserScore(gs.UserId, gs.Score, Game.Multiplier)).ToList();
                }
            }
        ...
        private static Game6_UserScore_Dto GenerateUserScore(string UserId, double Score, double Multiplier)
        {
            var res = new Game6_UserScore_Dto()
            {
                GameId = GameId,
