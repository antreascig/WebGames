...
{
    public class Game4_3_MetaData
    {
        //public double Multiplier { get; set; }
    }

    public class Game4_3_UserScore_Dto
    {
    ...
    public class Game4_3_Manager
    {
        public static string GameKey = "Game4_3";
        ...
        public static void SetUserScore(string UserId, string[] Stages, bool EnableOverride = false)
        {
            ...
            {
                var Entity = db.Game4_3_Scores.Find(UserId);
                if (Entity == null)
                {
                    Entity = new Models.Game4_3_UserScore()
                    {
                        UserId = UserId,
                        StagesCount = Stages.Length,
                        Stages = string.Join(",", Stages)
                    };
                    db.Entry<Models.Game4_3_UserScore>(Entity);
                }
        ...
        public static Game4_3_UserScore_Dto GetUserScore(string UserId)
        {
            var res = new Game4_3_UserScore_Dto();
            using (var db = ApplicationDbContext.Create())
            {
                var ScoreEntity = (from gs in db.Game4_3_Scores where gs.UserId == UserId select gs).ToList().FirstOrDefault();
                if (ScoreEntity == null)
                {
                    ScoreEntity = new Models.Game4_3_UserScore()
                    {
                        UserId = UserId,
                        Stages = "",
                        StagesCount = 0
        ...
        public static List<Game4_3_UserScore_Dto> GetUsersScore()
        {
            var res = new List<Game4_3_UserScore_Dto>();
            using (var db = ApplicationDbContext.Create())
            {
                var Game = GameHelper.GetGame(GameId, db);
                if (Game != null)
                {
                    res = (from gs in db.Game4_3_Scores select gs).Select(gs => GenerateUserScore(gs.UserId, gs.Stages, gs.StagesCount, Game.Multiplier)).ToList();
                }
            }
        ...
        private static Game4_3_UserScore_Dto GenerateUserScore(string UserId, string stages, int StagesCount, double Multiplier)
        {
